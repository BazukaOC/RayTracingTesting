...
public:
    RayTracer(vec3 start,
        ...
        ka_ = vec3(0.700, 0.600, 0.300);
        kd_ = vec3(0.700, 0.600, 0.300);
        ks_ = vec3(0.100, 0.100, 0.100);
...
Pixel RayTracer::phongModelColor(Data &data) {
    float nh = data.normal * data.half; nh = nh > 0 ? nh : 0;
    float Ia = 0.7;
    float Id = data.normal * data.hitpointToLight;
    float Is = pow(nh, exponential_);
    float intansity = 5.0;
